...
class RecordController extends Controller{
    ...
    public $daily_purchase_log_table = "ngw_click_log";

    public $goods_daily_report_table = "ngw_goods_daily_report";

    public $goods_shopping_log_table = "ngw_shopping_log";
    ...
    public function clickRecord($uid=null,$item_id=null){
        ...
        if($uid&&$item_id){
            ...
            if(!$rt||$this->redis==null){
                ...
                /*
                list($sql,$insert_data) = fetchInsertMoreSql("ngw_daily_uid_goods_report",array_keys($data),$click_data,false,$this->pdo);
    ...
    public function updateOrderInfo($order_list=array('7145541093113222')){
       ...
       // echo $sql;exit;
        ...
        $sql_list[] = "insert into ngw_order(status,cost,amount,uid,taobao_nick,createdAt,created_date,order_id,num_iid,coupon_id,title,pict_url,item_url,category,promotion_url,price,volume,
                rating,seller_id,seller_name,store_name,store_type,top,taobao_cid,ngw_pid,ngw_id,ngw_name,sum,num,val,limited,reduce,discount,
    ...
    //        $order_id_list = array(2937120419083222,2930383815453222,2930383815453223);
           ...
           //退单，买单都是必须存在的order信息，顺带记录进去 扣除邮费的成交价和利润
            /*$sql = "insert into ngw_shopping_log(status,uid,taobao_nick,fee,order_id,order_status,num_iid,report_date)
            select 1,uid,taobao_nick,deal_price fee,order_id,".$order_status.",num_iid,'".$this->date."' report_date from ngw_order
    ...
    public function readUidPurchaseInfo(){
        ...
        $sql_list[] = "delete from ngw_goods_daily_report where report_date = '".$this->date."' and click = 0";

        //购买成功的购买数据汇集，按商品聚合
        $sql_list[] = "insert ngw_goods_daily_report(report_date,num_iid,click,purchase,order_status)
            select report_date,num_iid,0,count(num_iid) purchase,order_status from ngw_shopping_log
    ...
    */
    ...
    public function _dailyReportByPurchase($date=null){
        ...
        $sql_list[] = "delete from ngw_goods_daily_report where report_date = '".$date."' and click = 0";
    ...
    public function _dailyReportByClick($date=null){
        ...
        $sql_list[] = "insert ngw_goods_daily_report(report_date,num_iid,click,purchase,order_status)
            select report_date,num_iid,sum(click) click,0,2 from ngw_click_log
