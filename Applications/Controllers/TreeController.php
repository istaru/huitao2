...
class TreeController
{
    ...
    public function addnode()
    {
        ...
        try {
            if(!empty($this->param['pid'])){
                ...
                $sql = "UPDATE ngw_category SET `left` = `left`+2 WHERE `left` >= {$p_info['right']} AND `type` = {$type}";
                M()->query($sql);
            }
    ...
    public function delnode()
    {
        ...
        M()->startTrans();
        try {
            $sql = "DELETE FROM ngw_category WHERE `left` >= {$node['left']} AND `right` <= {$node['right']}  AND `type` = {$this->param['type']}";
            M()->query($sql);

            $sql = "UPDATE ngw_category SET `left` = `left`-{$count}, `right` = `right`-{$count} WHERE `left` > {$node['left']}  AND `type` = {$this->param['type']}";
            M()->query($sql);

            $sql = "UPDATE ngw_category SET `right` = `right`-{$count} WHERE `right` > {$node['right']} AND `left` < {$node['left']}  AND `type` = {$this->param['type']}";
            M()->query($sql);
        } catch (Exception $e) {
            M()->rollback();
            echo 'fail';die;
