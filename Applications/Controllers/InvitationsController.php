...
class InvitationsController extends AppController
{
    ...
    public function rankingList() {
        ...
        if(!empty($params['user_id'])) {
            $self = M()->query("SELECT a.money,b.person_num FROM( SELECT sum(price) money , uid FROM ngw_uid_log WHERE status IN(1 , 2) AND uid = '{$params['user_id']}' and score_source is not null and score_source != '') a LEFT JOIN( SELECT count(0) person_num , uid FROM ngw_uid_log WHERE score_type = 2 AND uid = '{$params['user_id']}') b ON b.uid = a.uid", 'single');
        }
        if(empty($self) || !$self['money']) $self = ['money' => 0, 'person_num' => 0];
       //月榜
