...
class Incomes2Controller
{
	 ...
	 */
	public function buySuccess($order_list=['2623258984503529','3131590033367715'])
	{
		if(empty($order_list)) return;
		//查询订单信息提取提成
		$list = implode(',',$order_list);

		$sql = "select distinct(o.order_id) , o.uid , if(o.cost<o.deal_price,o.cost,o.deal_price) as cost , o.rating,o.amount,u.sfuid from gw_order o join gw_uid u on o.uid = u.objectId where o.order_id in ({$list}) and o.status = 1 ";
		$order_info = M()->query($sql,'all');
	 ...
	 */
	public function buyFail($order_list=['2623258984503529','3131590033367715'])
	{

		if(empty($order_list)) return;
		$list = implode(',',$order_list);

		$list = $this->checkBill($list);
	...
	public function checkBill($list)
	{
		$sql = "select distinct(order_id) from gw_uid_bill_log where status = 1 and order_id in ({$list})";
		$res = array_column(M()->query($sql,'all'),'order_id');

		$sql = "update gw_uid_bill_log set status = 3 where status = 1 and order_id in ({$list})";
		M()->query($sql);

		$res = array_diff($res,explode(',',$list));
		return implode(',',$res);
