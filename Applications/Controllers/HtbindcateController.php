    ...
    public  function getcategory(){
        ...
        if(isset($_REQUEST["type"])&&!empty($_REQUEST["type"])){
             if($_REQUEST["type"]=='fav'){
                 $sql="select favorite_id,favorite_name,category_id,category_name from gw_category_favorite_ref  where favorite_name like '%".addslashes($_REQUEST['keyword'])."%'";
                 $res_arr['fav']=M()->query($sql,'all');
                 $res_arr['fav']?info('ok',10,$res_arr):info("选品库暂无数据",-3);
             }else{
                 $sql="select pid,name from gw_category where name like '%".addslashes($_REQUEST['keyword'])."%' and name is not null GROUP BY name,pid ORDER BY pid ASC";
         ...
         */
            ...
            $sql2='select favorite_id,favorite_name,category_id,category_name from gw_category_favorite_ref';
    ...
    //将选品库分类绑定到惠淘自定义分类上
      public function bindcategory(){
          if(isset($_REQUEST['fid'])&&isset($_REQUEST['pid'])){
              $sql="update gw_category_favorite_ref set category_id='".$_REQUEST['pid']."',category_name=(select DISTINCT(name) from gw_category where pid='".$_REQUEST['pid']."') where favorite_id=".$_REQUEST['fid'];
              $res=M()->exec($sql);
