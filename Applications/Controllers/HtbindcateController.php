...
class HtbindcateController extends Controller
{
    ...
    public  function getcategory(){
        ...
        if(isset($_REQUEST["type"])&&!empty($_REQUEST["type"])){
            if($_REQUEST["type"]=='fav'){
                ...
                $sql="select id,name from ngw_category where name like '%".addslashes($_REQUEST['keyword'])."%' and name is not null GROUP BY name,id ORDER BY id ASC";
    ...
    public function bindcategory(){
        if(isset($_REQUEST['fid'])&&isset($_REQUEST['pid'])){
            $sql="update ngw_category_favorite_ref set category_id='".$_REQUEST['pid']."',category_name=(select DISTINCT(name) from ngw_category where id='".$_REQUEST['pid']."') where favorite_id=".$_REQUEST['fid'];
            $res=M()->exec($sql);
