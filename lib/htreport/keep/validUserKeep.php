...
class validUserKeep extends keepReport{
    public $pre_inner_sql = " select DISTINCT(a.uid) from ngw_click_log a join ";
	...
	public function __construct($time='',$media='',$type=''){
        ...
        if($media != ''){
            $this->inner_sql =  $this->pre_inner_sql." ngw_tracking b on a.uid = b.uid where a.report_date = '{$time}'  and b.source = '{$media}' ";
        }else{
            $this->inner_sql =  "select DISTINCT(uid) from ngw_click_log where report_date = '{$time}'";
        }
	}
