...
class newDeviceUserKeep extends keepReport{
    ...
    select b.uid from gw_did a join gw_taobao_log b on a.id = b.did_id where a.report_date = '%s'
) a join ";
    ...
    public function __construct($time='',$media='',$type){
        ...
        if($media != ''){
            $this->inner_sql =  $this->pre_inner_sql." gw_tracking b on a.uid = b.uid where b.report_date = '{$time}' and b.source = '{$media}' ";
        }else{
            $this->inner_sql =  $this->pre_inner_sql." gw_uid b on a.uid = b.objectId where b.report_date = '{$time}' ";
        }
    }
